...
bool IndexedFile::reloadFileProps(){
    ...
    if(!warnedOutsideFile && outsidefile){
        ...
        if(!NativeMessageBox::showOkCancelBox(AlertWindow::QuestionIcon,
                    ...
                    " and is also not zero, and considers this to be the end of " + fname + "--it's the first data "
...
uint32 IndexedFile::getRealEnd(ROM &rom, uint32 dataend){
    uint32 realend;
    for(realend = dataend; realend < rom.getSize(); ++realend){
...
bool IndexedFile::couldBeIndex(ROM &rom, int indextype, bool inorder, uint32 iaddr){
    uint32 rsz = rom.getSize();
    if(iaddr + 4 > rsz) return false;
    uint16 icount = rom.readHalfWord(iaddr + ((indextype == 2) ? 0 : 2));
    if(icount < 2 || icount > 1000) return false;
    if(indextype == 2){
        //Remainder of header should be empty
        if(iaddr + 16 >= rom.getSize()) return false;
        for(int i=2; i<16; i+=2){
            if(rom.readHalfWord(iaddr + i) != 0) return false;
        }
    }
    int it_hdrlen, it_entrylen;
    if(indextype == 2){
        it_hdrlen = 16;
        it_entrylen = 16;
    }else{
    ...
    uint32 lastoend = 0;
    for(uint16 o=0; o<icount; ++o){
        uint32 ientryaddr = iaddr + (it_entrylen*o) + it_hdrlen;
        if(ientryaddr + 8 > rsz) return false;
        uint32 oaddr = rom.readWord(ientryaddr);
        uint32 olen = rom.readWord(ientryaddr + 4);
        if(olen == 0){
            if(oaddr >= icount) return false; //invalid pointer
            if(oaddr == o) return false; //self-referential pointer
        }else{
            if(oaddr >= rsz || olen >= rsz || oaddr + olen > rsz) return false;
            if(inorder && oaddr < lastoend) return false;
            lastoend = oaddr + olen;
        }
    }
    return true;
